...
| 2 | 3 reviewers re-checking every round-1 fix commit (core util/io/http, core server/client, EE/build/docs; reports in `review/round2/`) | 0 / 1 / 5 / ~10 — all round-1 findings confirmed fixed (8 agreed false positives); medium: the `String.format` layout pass reformatted 4 pre-existing (unported) sites; lows: dead null-`ClassLoader` branch left in `ResourceFactory`, core websocket `getCookies()` not unmodifiable, 3 test-distribution `String.format` layouts, status-file record list incomplete, dead `lucene.version` property, njord/mimir leftovers in the release script and CI | all fixed in commit 90851c7ed34 (full checkstyle+spotless build green on JDK 11), round 3 re-review |
| 3 | 2 reviewers re-checking the round-2 fix commit (core, EE/build/docs) | in progress | |
