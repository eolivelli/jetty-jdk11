...
| 2 | 3 reviewers re-checking every round-1 fix commit (core util/io/http, core server/client, EE/build/docs; reports in `review/round2/`) | 0 / 1 / 5 / ~10 — all round-1 findings confirmed fixed (8 agreed false positives); medium: the `String.format` layout pass reformatted 4 pre-existing (unported) sites; lows: dead null-`ClassLoader` branch left in `ResourceFactory`, core websocket `getCookies()` not unmodifiable, 3 test-distribution `String.format` layouts, status-file record list incomplete, `HttpSpiContextHandler` exception precedence when both the body and `close()` fail (optional); nits: dead `lucene.version` property, njord/mimir leftovers in the release script and CI, `%n` in a JMH diagnostic | all fixed in commit 90851c7ed34 (full checkstyle+spotless build green on JDK 11) except the two accepted-as-is items (`HttpSpiContextHandler` exception precedence, `%n` in the JMH benchmark); round 3 re-review |
| 3 | 2 reviewers re-checking the round-2 fix commit (core, EE/build/docs; reports in `review/round3/`) | 0 / 0 / 1 / 4 — core: every round-2 finding fixed, nothing new, both optional nits explicitly accepted; EE/build/docs: all fixed, one low (wording of this table's round-2 row), nits: inert `maven.build.cache` flags left in `Jenkinsfile`/`release-jetty.sh`, a doubled blank line, timestamps unusable as build evidence (reproducible archives) | fixed in the next commit (table reworded, build-cache flags and `useBuildCache()` removed); round 4 re-review of that commit |
| 4 | 1 reviewer re-checking the round-3 fix commit | in progress | |
