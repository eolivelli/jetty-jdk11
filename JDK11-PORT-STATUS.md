...
| 7 | EE10 / EE9 / EE8 test suites on JDK 11 | ✅ | Final re-run on the reviewed tree (fcde796d375, jetty-core jars from the round-2 fix build): EE10 3704 tests, EE9 4541 tests — only the known environmental/timing failures (`ProxyWebAppTest`, `*DoSFilterTest.testBurstLowRateIP`, the H3 variants, the load-sensitive `AsyncIOServletTest.testAsyncReadEcho`, plus two ee10 `jakarta` websocket tests, `AnnotatedServerEndpointTest.testOverrideEndpointConfig` and `SessionTrackingTest.testAddRemoveSessions`, and the H2 variant of `HttpClientContinueTest.testExpect100ContinueWithContentWithResponseFailureAfter100Continue`, all of which failed once under the parallel `-T 2` run and pass in isolation). Earlier re-run after the round-1 review fixes (2026-09-17, worktree at 8edce235bfe with jetty-core jars built 23:34, i.e. before the round-1 cosmetic pass aae0bf19f91 (two behaviour-neutral jetty-core hunks) and the round-2 fix commit 90851c7ed34, whose jetty-core changes are compile-verified by the full JDK 11 build and reviewed in round 3): EE10 3705 tests, EE9 4541 tests — same environmental/timing failures only (earlier batched runs: EE10 ~2700, EE9 ~2700, EE8 3164 tests). Remaining failures are all environmental or timing-related, none deterministic on JDK 11: `ProxyWebAppTest` (proxies to https://javadoc.jetty.org, unreachable here), `*DoSFilterTest.testBurstLowRateIP` (wall-clock rate limiting), `DeploymentErrorTest.testDelayedAddBadAppUnavailableFalse` and ee9 `RequestTest` (order/timing dependent, pass in isolation), websocket `IdleTimeoutTest`/`LargeAnnotatedTest` and the ee10/ee9 `AsyncIOServletTest` echo tests (1M small writes against a 30 s latch; fail only when the machine is loaded by a parallel build, pass in isolation), and the HTTP/3 (`H3_QUICHE`) variants of the EE client-transport tests failing with quiche `UNKNOWN_CA` (or, for `NeedClientAuthTest`, an NPE on the missing client `SslContextFactory`) — reproduced identically on JDK 17 with the untouched upstream sources on this machine → pre-existing, not caused by the port. Integrations/demos/tests batch (66 modules): 206 tests, 0 failures |
...
- The Mimir/Njord Maven extensions (Java 17+) are removed together with their leftovers: the CodeQL workflow
  ...
  `njord:publish` after `mvn deploy` (`mvn deploy -Peclipse-release` targets `repo.maven.apache.org` directly and will not
  work without the extension, so publishing to Central must be done by other means on this branch). The inert
  `maven.build.cache.*` flags and the `useBuildCache()` logic of the removed build-cache extension are dropped as well;
  `-Dmaven.test.failure.ignore=true` is still passed only on `jetty-12.1.x` and on PRs labelled `build-all-tests` or
  `build-no-cache`, as before.
...
| 2 | 3 reviewers re-checking every round-1 fix commit (core util/io/http, core server/client, EE/build/docs; reports in `review/round2/`) | 0 / 1 / 5 / ~10 — all round-1 findings confirmed fixed (8 groups — 14 files, 18 sites — of agreed false positives); medium: the `String.format` layout pass reformatted 4 pre-existing (unported) sites; lows: dead null-`ClassLoader` branch left in `ResourceFactory`, core websocket `getCookies()` not unmodifiable, 3 test-distribution `String.format` layouts, status-file record list incomplete, `HttpSpiContextHandler` exception precedence when both the body and `close()` fail (optional); nits: dead `lucene.version` property, njord/mimir leftovers in the release script and CI, `%n` in a JMH diagnostic, … | all fixed in commit 90851c7ed34 (full checkstyle+spotless build green on JDK 11) except the four accepted-as-is items (`HttpSpiContextHandler` exception precedence, `%n` in the JMH benchmark, `PersistAuthenticationCredentialsTest` hoisted local, `ContextHandlerCollectionTest` default-branch exception type); round 3 re-review |
| 3 | 2 reviewers re-checking the round-2 fix commit (core, EE/build/docs; reports in `review/round3/`) | 0 / 0 / 1 / 4 — core: every round-2 finding fixed, nothing new, both optional nits explicitly accepted; EE/build/docs: all fixed, one low (wording of this table's round-2 row), nits: inert `maven.build.cache` flags left in `Jenkinsfile`/`release-jetty.sh`, a doubled blank line, timestamps unusable as build evidence (reproducible archives), plus an informational note on the `ArrayByteBufferPool` argument grouping | fixed in the next commit (table reworded, build-cache flags and `useBuildCache()` removed); round 4 re-review of that commit |
| 4 | 1 reviewer re-checking the round-3 fix commit (report in `review/round4/`) | 0 / 0 / 3 / 2 — lows: the step-7 note overstated which tree the EE re-run exercised, the `Jenkinsfile` passed `-Dmaven.test.failure.ignore=true` unconditionally (base only did so on `jetty-12.1.x` / labelled PRs) leaving a dead label check, round-2 row miscounted the accepted-as-is items; nits: duplicated EE totals / `NeedClientAuthTest` H3 failure mode, row-3 nit count not identifiable from its text | fixed in the next commit (Jenkinsfile rule restored without the cache, table/notes reworded); round 5 re-review |
| 5 | 1 reviewer re-checking the round-4 fix commit (report in `review/round5/`) | 0 / 0 / 0 / 3 — the `Jenkinsfile` rule verified case by case against the base, all round-4 items fixed; only optional wording nits (jar generation also predates the round-1 cosmetic pass, false-positive site count, row-4 nit naming) | **reviewers satisfied — no actionable finding left**; the wording nits are applied in the final status commit |
