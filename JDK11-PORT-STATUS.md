...
| 10 | test-distribution, maven-plugin ITs, checkstyle/spotless re-enabled for the final build | 🔄 | **final `mvn clean install -DskipTests` on JDK 11 with checkstyle and spotless enabled: BUILD SUCCESS, 364 modules**. test-distribution: 224 tests, 28 failures on the first run (parameterized tests broken by the ee11 removal, deleted Servlet 6 demo war, Unix-Domain test needing a JDK 16 gate, WildFly Elytron jar for the Infinispan Hot Rod module, plus environmental download test) — being fixed in branch `jdk11/dist`. maven-plugin invoker ITs on JDK 11: jetty-maven-plugin 13/13 for each of ee10/ee9/ee8, jspc-maven-plugin 3/3 each — all green |
